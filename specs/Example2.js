...
describe('Protractor Demo App', function () {
  ...
  it('Display the price based on the item', function () {
    ...
    table.all(by.tagName("tr")).each(function (row) {
      row.all(by.css("td:nth-child(3) strong")).getText().then(function (cellVal) {
        if (cellVal.length != 0) {
          sum = sum + Number(cellVal.toString().split(".")[1].trim());
        }
      })
    }).then(function () {
      console.log("============" + sum);
  ...
  fit('Check that after deletion of the item== Samsung Note 8 == its not available in cart', function () {
    browser.get('https://qaclickacademy.github.io/protocommerce/');
    browser.driver.manage().window().maximize();
    element(by.linkText("Shop")).click();
    addItemToCart("Samsung Note 8");
    ...
    table.all(by.tagName("tr")).each(function (row) {
      row.all(by.css("td:nth-child(1) h4 a")).getText().then(function (cellVal) {
        if (cellVal == 'Samsung Note 8') {
          row.element(by.css("td:nth-child(5) button")).click();
        }
      })
    });
